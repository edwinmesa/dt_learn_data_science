    ...
    def insert_edge(self, new_edge_val, node_from_val, node_to_val):
        ...
        for node in self.nodes:
            ...
            if node_to_val == node.value:
                to_found = node
        if from_found == None: # first loop insert: 1
            from_found = Node(node_from_val)
            self.nodes.append(from_found)
        if to_found == None:  # first loop insert: 2
            to_found = Node(node_to_val)
            self.nodes.append(to_found)
        new_edge = Edge(new_edge_val, from_found, to_found) # insert: (100,1,2)
        from_found.edges.append(new_edge)                   # 1
        to_found.edges.append(new_edge)                     # 2
